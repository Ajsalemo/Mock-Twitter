// --------------------------------------------- Imports ----------------------------------------------- //
// ----------------------------------------------------------------------------------------------------- //
// This file was automatically generaled from the graphcool/auth0-template
  ...
  new Promise(resolve => {
    ...
    jkwsClient.getSigningKey(decoded.header.kid, (err, key) => {
      ...
      jwt.verify(
        ...
        {
          algorithms: ['RS256'],
          aud: process.env.REACT_APP_AUTH0_API_IDENTIFIER,
          ignoreExpiration: false,
          issuer: `${process.env.REACT_APP_AUTH0_API_IDENTIFIER}`
        },
        (err, decoded) => {
          if (err) throw new Error(err)
